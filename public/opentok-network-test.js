...
!(global => {
  function OTNetworkTest(options) {
    ...
    const testStreamingCapability = (subscriber, callback) => {
      performQualityTest(
        ...
        (error, results) => {
          ...
          if (!results.video) {
            ...
            if (audioSupported) {
              return callback(false, {
                text:
                  "You can't send video because no camera was found, " +
          ...
          const audioVideoSupported =
            results.video.bitsPerSecond > 250000 &&
    ...
    this.startNetworkTest = callback => {
      ...
      const callbacks = {
        onInitPublisher: function onInitPublisher(error) {
          if (error) {
            console.error(
              'Could not acquire your camera and microphone.',
              error
            );
            return;
          }
    ...
    function calculatePerSecondStats(statsBuffer, seconds) {
      const stats = {};
      const activeMediaTypes = Object.keys(statsBuffer[0] || {}).filter(key => {
      ...
      activeMediaTypes.forEach(type => {
        stats[type] = {
          packetsPerSecond:
            sum(pluck(statsBuffer, type), 'packetsReceived') / seconds,
          bitsPerSecond:
            (sum(pluck(statsBuffer, type), 'bytesReceived') * 8) / seconds,
          packetsLostPerSecond:
            sum(pluck(statsBuffer, type), 'packetsLost') / seconds
        };
        ...
        start(reportFunction) {
          ...
          getStatsIntervalId = window.setInterval(() => {
            config.subscriber.getStats((error, stats) => {
              const activeMediaTypes = Object.keys(stats).filter(key => {
                return key !== 'timestamp';
              });
              const snapshot = {};
              ...
              activeMediaTypes.forEach(type => {
                snapshot[type] = Object.keys(stats[type]).reduce(
                  (result, key) => {
